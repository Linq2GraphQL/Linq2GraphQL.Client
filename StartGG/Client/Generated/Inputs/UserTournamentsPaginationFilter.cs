//---------------------------------------------------------------------
// This code was automatically generated by Linq2GraphQL
// Please don't edit this file
// Github:https://github.com/linq2graphql/linq2graphql.client
// Url: https://linq2graphql.com
//---------------------------------------------------------------------

using System;
using System.Collections.Generic;
using System.Text.Json.Serialization;
using Linq2GraphQL.Client;

namespace StartGG.Client;

[JsonConverter(typeof(GraphInputConverter<UserTournamentsPaginationFilter>))]
public partial class UserTournamentsPaginationFilter : GraphInputBase
{
	[GraphQLMember("past")]
	[JsonPropertyName("past")]
	public bool? Past 
	{
		get => GetValue<bool?>("past");
    	set => SetValue("past", value);
	}

	[GraphQLMember("upcoming")]
	[JsonPropertyName("upcoming")]
	public bool? Upcoming 
	{
		get => GetValue<bool?>("upcoming");
    	set => SetValue("upcoming", value);
	}

	[GraphQLMember("search")]
	[JsonPropertyName("search")]
	public PaginationSearchType Search 
	{
		get => GetValue<PaginationSearchType>("search");
    	set => SetValue("search", value);
	}

	[GraphQLMember("videogameId")]
	[JsonPropertyName("videogameId")]
	public List<ID> VideogameId 
	{
		get => GetValue<List<ID>>("videogameId");
    	set => SetValue("videogameId", value);
	}

	[GraphQLMember("tournamentView")]
	[JsonPropertyName("tournamentView")]
	public string TournamentView 
	{
		get => GetValue<string>("tournamentView");
    	set => SetValue("tournamentView", value);
	}

	[GraphQLMember("excludeId")]
	[JsonPropertyName("excludeId")]
	public List<ID> ExcludeId 
	{
		get => GetValue<List<ID>>("excludeId");
    	set => SetValue("excludeId", value);
	}

}